...
class MainPage implements InterfaceContainerElement {
  ...
  private createUsersMixesCard() {
    const usersMixesCard = createHTMLElement('users-mixes');
    usersMixesCard.onclick = (event) => this.openUsersMixes();
    const title = createHTMLElement('users-mixes__title', 'h4');
    title.textContent = 'Миксы пользователей';
    usersMixesCard.appendChild(title);
    const tryButton = createHTMLElement(['button-1', 'users-mixes__button'], 'button');
    tryButton.textContent = 'ПРОБОВАТЬ';
    usersMixesCard.appendChild(tryButton);
    return usersMixesCard;
  }

  private openUsersMixes() {
    /*
    TO-DO: добавить открытие раздела с миксами от пользователей
    */
  }
